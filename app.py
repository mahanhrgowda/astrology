...
# Siddha Force mapping (variant system with adjusted element associations)
...
# Siddha Force descriptions (long divination texts for the variant system)
siddha_descriptions = {
...
# Main app
st.title("Astrology & Divination by Mahaan 🔮🦅🌟")
...
place = st.text_input("Birth Place 🏙️ (Optional, for display)")
...
if st.button("Generate Insights ✨"):
    ...
    # Randomize string mapping each time (from first app)
    ...
    # Siddha Pakshi Force (variant system)
    siddha_force = get_siddha_force(nak_name, paksha)
    divination_desc = siddha_descriptions.get(siddha_force, "Mysterious forces guide your path! 🌌")
    ...
    st.write(f"- **Siddha Pakshi Force (Variant System):** {siddha_force}")
    ...
    st.write(f"**Siddha Pakshi Prasna Divination (Variant System):** {divination_desc}")
    ...
    with st.expander("Meanings of All Siddha Forces (Variant System)"):
        for force, desc in siddha_descriptions.items():
